...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 20 12:26:30 2018
...
class CovGreedy(Greedy):
    def initializeS(self):
        ...
        for j in range(self.n):
            self.invA0=shermanMorissonUpdate(self.invA0,self.X_abs[j,:])
    def updateS(self,key):
...
class MutGreedy(Greedy):
    def initializeS(self):
        Greedy.initializeS(self)
        #self.Remainder=random.sample(self.Remainder,10000)
...
class FishGreedy(Greedy):
    def initializeS(self,delta=0.001):
        ...
        for j in range(N_item):
            ...
            x_jj=np.dot(self.Xnorm[j,:],self.Xnorm[j,:])
            self.gs_UmatX[j]=1/(1/self.gs_Umat[j]+x_jj**2)
    ...
    def greedyGrow(self,k):
        ...
        while len(self.S)<k:
            ...
            for i in self.Remainlist:
                ...
                x_si=np.dot(self.Xnorm[i,:],self.Xnorm.T[:,item_minx])
                x_si2=np.multiply(x_si,x_si)
                val=self.gs_Umat[i]+np.dot(self.gs_Umat[item_minx],x_si2)*self.gs_UmatX[i]
                if val>maxVal:
                    maxVal=val
                    opti = i        
            key_opti=self.Remainkey[opti]
            self.Remainlist.remove(opti)
            self.S.append(key_opti)
            for j in self.Remainlist:
                x_jo=np.dot(self.Xnorm[j,:],self.Xnorm[opti,:])
                x_jj=np.dot(self.Xnorm[j,:],self.Xnorm[j,:])
                self.gs_Umat[j]=1/(1/self.gs_Umat[j]+self.P[opti]/self.P[j]*(x_jo**2))
                self.gs_UmatX[j]=1/(1/self.gs_Umat[j]+x_jj**2)
            time2=time.time()
            Time.append(time2-time1)
            time1=time.time()
